...
class TalkController
  ...
  def add_talk_to_db(talk_data)
    ...
    if check_if_time_is_taken(new_talk, search_talks) == true
      @talk_view.already_a_talk
    else
      @talk_repository.create(new_talk)
      @talk_view.talk_created(talk_data[0], talk_data[1].name, convert_time(talk_data[2]), convert_time(talk_data[3]), talk_data[4].name)
    end
  end
  ...
  def print_talks(event)
    ...
    return @talk_view.event_not_found if all_talks == []
    ...
    all_talks.each do |talk|
      ...
      @talk_view.show_all_talks(talk_name, convert_time(start_time), convert_time(end_time), speaker_name)
    end
  end
